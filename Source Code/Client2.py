...
if __name__ == "__main__":
    client2 = ThreadedServer(Client2, hostname=SERVER, port=PORT2)
    client_server_thread = Thread(target=client2.start)
    ...
    while True:
        ...
        if selection == '1':
            ...
            while len(Files) < Fileslen:
                ...
                Files.append(File)
            print(conn.root.exposed_Register(PORT2, Files))
            Files = []
        elif selection == '2':
            ...
            conn.root.exposed_Terminate(PORT2)
            #quit()
